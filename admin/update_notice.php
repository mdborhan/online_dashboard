...
if(isset($update))
{
mysqli_query($myconn,"update notice set subject='$sub',Description='$details' where notice_id='".$_GET['notice_id']."'");
...
$q=mysqli_query($myconn,"select * from notice where notice_id='".$_GET['notice_id']."'");
...
<form method="post">
	...
	<div class="row">
		...
		<select name="user[]" multiple="multiple" class="form-control">
			<?php 
	$sql=mysqli_query($myconn,"select name,email from user");
